...
def click_event (i,j):                                                             #Function which is used to get the pixel values and convert them to get greyscale value where the range we are using is being sent from the part of the code where the function is being called
 ...
 global Black_Total_Dice

 global White_Total_Dice
 ...
 if(gray<16):                                                                      # if the value of gray is less than 43 a dice with one dot would be drawn on the screen

    draw.rectangle((q,l,q1,l1), fill=(bg_color), outline=(color))                                 # Using the pil lib's function first we are drawing a rectangle using the value of coordinates represented by q and l 
    r=2                                       
    ...
    draw.ellipse((aa-r,bb-r,aa+r, bb+r), fill=(color))                           # This function is used to make a dot at the desired coordinate or we can say centre 

    White_Total_Dice = White_Total_Dice + 1

 elif(gray<32):

    draw.rectangle((q,l,q1,l1), fill=(bg_color), outline=(color))
    r=2
    aa=(q+q1)/2
    bb=(l+l1)/2
    ...
    ee=(q1+aa)/2
    ff=(l1+bb)/2
    draw.ellipse((cc-r,dd-r,cc+r, dd+r), fill=(color))                           # a dot with white color at the specified position will be made   
    draw.ellipse((ee-r,ff-r,ee+r, ff+r), fill=(color))
    ...
    ee=(q1+aa)/2
    ff=(l1+bb)/2
    draw.ellipse((cc-r,dd-r,cc+r, dd+r), fill=(color))
    draw.ellipse((ee-r,ff-r,ee+r, ff+r), fill=(color))
    draw.ellipse((aa-r,bb-r,aa+r, bb+r), fill=(color))
    ...
    draw.ellipse((cc-r,dd-r,cc+r, dd+r), fill=(color))
    draw.ellipse((ee-r,ff-r,ee+r, ff+r), fill=(color))
    draw.ellipse((gg-r,hh-r,gg+r, hh+r), fill=(color))
    draw.ellipse((ii-r,jj-r,ii+r, jj+r), fill=(color))
 ...
 elif(gray<96):                                                                      # if the value of gray is less than 43 a dice with one dot would be drawn on the screen
    ...
    draw.ellipse((gg-r,hh-r,gg+r, hh+r), fill=(color))
    draw.ellipse((ii-r,jj-r,ii+r, jj+r), fill=(color))
    draw.ellipse((cc-r,bb-r,cc+r, bb+r), fill=(color))
    draw.ellipse((gg-r,bb-r,gg+r, bb+r), fill=(color))

    White_Total_Dice = White_Total_Dice + 1
    
 elif(gray<144):                                                                    # The same procedure is repeated for dice with two dots 
    ...
    Black_Total_Dice = Black_Total_Dice + 1
     

 elif(gray<160):                                                                    # The same procedure is repeated for dice with three dots 
 ...
 elif(gray<176):                                                                     # The same procedure is repeated for dice with four dots 
 ...
 elif(gray<192):                                                                      # The same procedure is repeated for dice with five dots 
    ...
    draw.rectangle((q,l,q1,l1), fill=(color), outline=(bg_color))                                 # Using the pil lib's function first we are drawing a rectangle using the value of coordinates represented by q and l 
    r=2                                       
    aa=(q+q1)/2                                                                    # used to calculate the mid point of coordinated denoted by q and q1
    bb=(l+l1)/2                                                                    # used to calculate the mid point of coordinated denoted by 1 and l1
...
img=cv2.imread('MOELogo70.jpg')                                                  # The image is being read or loaded through this statement 
...
print("Total Number of Black Dice:",Black_Total_Dice)                               
print("Total Number of White Dice:",White_Total_Dice)                               


im.save('whitedice_blackdots_ima.png', quality=100)                                           # The output is saved in the file so named here 
im.save('whitedice_blackdots_ima.eps', quality=100)                                           # The output is saved in the file so named here 
cv2.waitKey(0)
